# ══════════════════════════════════════════════════════════════════════════════
# Vegetable_Final_v2.py
# Upgrades over v1:
#   1. Price lag features  : lag_14, lag_30 added  (already had 1, 3, 7)
#   2. Rolling price stats : rolling_median_7, rolling_mean_14, rolling_mean_30
#   3. Price momentum      : pct_change_7, pct_change_30 added (already had 3)
#   4. Price volatility    : volatility_14, volatility_30 added (already had 7)
#   5. Arrival features    : arrival_rolling_14, arrival_rolling_30 added
#   6. Fuel momentum       : diesel_pct_change_7, petrol_pct_change_7
#   7. Weather interaction : rainfall_shock_7d, temp_deviation
#   8. Time features       : DayOfYear, WeekOfYear added
#   9. Price regime        : price_vs_30d_mean (how far from 30-day average)
#  10. Bug fixes           : .ffill() replaces deprecated fillna(method='ffill')
#                            all lags verified per commodity (no contamination)
...
# NEW: Rainfall shock — deviation from 7-day average (normalised)
...
# NEW: Temperature deviation from 14-day average
...
# Zero arrival flag (matches cereal pipeline)
...
# v1 had lag_1, 3, 7 — adding lag_14 and lag_30
...
# NEW: Rolling median (robust to spikes — shift(1) prevents leakage)
...
# Existing volatility_7 + NEW volatility_14 and volatility_30
...
# NEW: Price regime indicator — how far is today from 30-day average
...
# v1 had pct_change_3 — adding 7 and 30
...
# NEW: arrival_rolling_14 and arrival_rolling_30
...
# NEW: Supply shock — 7-day vs 30-day arrivals
...
lag_cols = [
    ...
    "arrival_shock", "supply_tightness", "supply_shock_7v30",
]
...
#             Vegetables have ~3,800 rows per commodity, so losing ~30 warmup
#             rows is only ~0.8% — completely negligible.
#   Step 2 — Backfill remaining NaNs within each commodity.
#             These are short-lag warmup rows where lag_30 is valid
#             but rolling(14).std() etc. still need more history.
#   Step 3 — Per-commodity median fill as final safety net.

veg_df = veg_df.sort_values(["commodity", "date"])

# Step 1: Drop rows where the longest warmup lag is NaN
...
#   bfill fills early NaNs using the NEXT available value going backward,
#   which is the first real computed value — best approximation for warmup rows.
veg_df[lag_cols] = (
    veg_df.groupby("commodity")[lag_cols]
    .transform(lambda x: x.bfill())
)

# Step 3: Per-commodity median fill for any remaining edge case NaNs
veg_df[lag_cols] = (
    veg_df.groupby("commodity")[lag_cols]
...
veg_df.to_csv("Agri-Hotri_Price_Prediction/Data/Vegetable_Final_v2.csv", index=False)
print("\n Saved -> Agri-Hotri_Price_Prediction/Data/Vegetable_Final_v2.csv")