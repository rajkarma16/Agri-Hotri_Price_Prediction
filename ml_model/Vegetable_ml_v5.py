"""
Vegetable Price Prediction — ML Pipeline v5
════════════════════════════════════════════════════════════
Changes from v4:
  ...
  2. Best-model feature importance (new Fig 9)
  ...
  3. best_params_log dict — stores every model's best hyperparams per commodity
...
# ---------------------------------------------
print("=" * 65)
print("  VEGETABLE PRICE PREDICTION — ML PIPELINE v5")
...
def build_models():
    ...
    return {
        ...
        'Extra Trees':
            ExtraTreesRegressor(n_estimators=600, max_depth=8,
                                min_samples_split=10, min_samples_leaf=4,
                                max_features='sqrt', bootstrap=True,
                                random_state=42, n_jobs=-1),

        'Gradient Boosting':
            GradientBoostingRegressor(n_estimators=600, learning_rate=0.02,
                                      max_depth=3, subsample=0.8,
                                      min_samples_leaf=5, random_state=42),

        'Hist Gradient Boosting':
            HistGradientBoostingRegressor(max_iter=400, learning_rate=0.03,
...
PARAM_GRIDS = {
    'Ridge Regression': {
        'alpha': [0.01, 0.1, 1, 5, 10, 50, 100, 200, 500],
    },
    'Lasso Regression': {
        'alpha':    [0.001, 0.01, 0.1, 0.5, 1, 5, 10, 50],
...
# ══════════════════════════════════════════════════════════
# FIG 9 — Best Model Feature Importances  ◄ NEW in v5
...
# ══════════════════════════════════════════════════════════
fig9, axes = plt.subplots(1, 3, figsize=(28, 11))
...
plt.tight_layout()
plt.savefig('./Agri-Hotri_Price_Prediction/outputs/Vegetable/veg_fig9_best_model_feature_importance.png',
            dpi=150, bbox_inches='tight', facecolor=BG_DARK)
plt.close(); print("   veg_fig9_best_model_feature_importance.png")

# ══════════════════════════════════════════════════════════
# FIG 10 — Hyperparameter Tuning Summary  ◄ NEW in v5
# ══════════════════════════════════════════════════════════
fig10, axes = plt.subplots(1, 3, figsize=(26, 8))
fig10.patch.set_facecolor(BG_DARK)
fig10.suptitle('Hyperparameter Tuning Summary — Val R² vs Test R² per Model\n'
...
plt.tight_layout()
plt.savefig('./Agri-Hotri_Price_Prediction/outputs/Vegetable/veg_fig10_tuning_summary.png',
            dpi=150, bbox_inches='tight', facecolor=BG_DARK)
plt.close(); print("   veg_fig10_tuning_summary.png")
