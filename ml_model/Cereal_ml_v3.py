"""
Cereal Price Prediction — ML Pipeline v3
════════════════════════════════════════════════════════════
Changes from v2:
  1. Reads Cereal_Final_v2.csv  (NaNs pre-filled, no imputer needed)
  2. NUMERIC_FEATURES expanded with all new engineered features:
       ...
       arrival_rolling_7, arrival_lag_3, arrival_shock, supply_tightness
       diesel_pct_change_7, petrol_pct_change_7
       rainfall_shock_30d
       DayOfYear, WeekOfYear
  3. SimpleImputer removed — replaced with NaN assertion guard
  4. season_enc no longer re-computed (already in v2 CSV)
  5. EDA heatmap updated to show new key features
  6. Feature list auto-filters to only columns present in CSV
...
# ─────────────────────────────────────────────
print("=" * 65)
print("  CEREAL PRICE PREDICTION — ML PIPELINE v3")
print("  Split: 70% Train | 15% Validation | 15% Test")
print("=" * 65)

df = pd.read_csv('Agri-Hotri_Price_Prediction/Data/Cereal_Final_v2.csv',
                 parse_dates=['date'])
...
# season_enc is now pre-computed in v2 CSV — no need to recompute here
# (v1 computed it here; removed in v3 to avoid double-encoding)
...
NUMERIC_FEATURES_WANTED = [
    ...
    'price_above_msp', 'msp_yearly_growth',           # new in v2
    # ── Fuel
    'Petrol_price', 'Diesel_price',
    'diesel_lag_7', 'diesel_lag_30', 'petrol_lag_7', 'petrol_lag_30',
    'diesel_pct_change_30', 'diesel_pct_change_7', 'petrol_pct_change_7',  # last 2 new
    # ── Time
    'Year', 'Month_Num', 'DayOfYear', 'WeekOfYear', 'season_enc',          # last 3 new
    # ── Flags
    'zero_arrival_flag', 'market_closed_flag',
    # ── Arrival features
    'arrivals_lag_7', 'arrivals_pct_change_7',
    'arrival_rolling_14', 'arrival_rolling_7', 'arrival_lag_3',            # last 2 new
    'supply_stress_index', 'arrival_shock', 'supply_tightness',            # last 2 new
    # ── Price lags  (ALL new in v2 — cereal v1 had none)
    'price_lag_1', 'price_lag_3', 'price_lag_7', 'price_lag_14', 'price_lag_30',
    # ── Rolling price stats  (ALL new in v2)
    'price_rolling_median_7', 'price_rolling_mean_14', 'price_rolling_mean_30',
    # ── Price volatility  (ALL new in v2)
    'price_volatility_7', 'price_volatility_30',
    # ── Price momentum  (ALL new in v2)
    'price_pct_change_3', 'price_pct_change_7', 'price_pct_change_30',
]

# Auto-filter to only columns that actually exist in this CSV
...
if missing:
    ...
    print("    -> Run Cereal_Final_v2.py first to generate all features")
else:
