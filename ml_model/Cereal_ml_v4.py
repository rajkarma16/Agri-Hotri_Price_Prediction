"""
Cereal Price Prediction — ML Pipeline v4
════════════════════════════════════════════════════════════
Changes from v3:
  1. Reads Cereal_Final_v3.csv
  ...
  3. NUMERIC_FEATURES expanded with all v3 engineered features:
       price_volatility_14, price_vs_30d_mean
       arrival_rolling_30, supply_shock_7v30
  ...
  5. New Fig 9 — Lasso Coefficients: top positive & negative feature weights
     (shows which features Lasso drives to zero = implicit feature selection)
  6. Fig 8 leaderboard now shows 6 models
...
# ─────────────────────────────────────────────
print("=" * 65)
print("  CEREAL PRICE PREDICTION — ML PIPELINE v4")
...
NUMERIC_FEATURES_WANTED = [
    # ── Weather (short-window — shared with Vegetable pipeline)
    'arrivals', 'temperature', 'rainfall', 'solar_radiation', 'wind_speed',
    'rainfall_7d', 'rainfall_15d', 'temp_7d_avg',
    'rainfall_shock_7d', 'temp_deviation_14d',
    # ── Weather (long-window — cereal crop cycle)
    'rainfall_30d', 'rainfall_60d', 'temp_14d_avg',
    'rainfall_shock_30d',
    # ── MSP (cereal-exclusive)
...
def build_models():
    return {
        ...
        # Mild boosting (only one tree model)
        'Gradient Boosting':
        ...
        # Regularized histogram boosting
...
for commodity in COMMODITIES:
    ...
    np.abs(fitted['Lasso Regression'].coef_),
